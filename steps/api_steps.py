...
@keyword_step(parsers.parse('I set the request header "{key}" to "{value}"'))
def i_set_the_request_header(key: str, value: str) -> None:
    _api_methods.set_header(key, value)


@keyword_step(parsers.parse('I set the path parameter "{key}" to "{value}"'))
def i_set_the_path_parameter(key: str, value: str) -> None:
    _api_methods.set_path_parameter(key, value)


@keyword_step(parsers.parse('I set the query parameter "{key}" to "{value}"'))
...
@keyword_step(parsers.parse('I send a "{request_key}" request to the "{service_name}" service'))
...
@keyword_step(parsers.parse("the response code should be {expected_status_code:d}"))
...
@keyword_step(parsers.parse('the response body should contain the text "{expected_text}"'))
...
@keyword_step(parsers.parse('the response header "{header_name}" should be "{expected_value}"'))
...
@keyword_step(parsers.parse('the value of the JSON path "{json_path}" should be "{expected_value}"'))
